/*!
 * @nuxt/server-edge v2.16.0-27358576.777a4b7f (c) 2016-2022
...
const errorMiddleware = ({ resources, options }) => async function errorMiddleware(_error, req, res, next) {
  ...
  const youch = new Youch__default["default"](error, req, readSource, options.router.base, true);
  if (isJson) {
...
const createTimingMiddleware = (options) => (req, res, next) => {
  ...
  onHeaders__default["default"](res, () => {
    ...
    if (res.timing.headers.length > 0) {
      res.setHeader("Server-Timing", [].concat(res.getHeader("Server-Timing") || []).concat(res.timing.headers).join(", "));
    }
    res.timing.clear();
  });
  next();
...
class Server {
  ...
  async setupMiddleware() {
    ...
    if (this.options.render.static !== false) {
      const staticMiddleware = serveStatic__default["default"](path__default["default"].resolve(this.options.srcDir, this.options.dir.static), this.options.render.static);
    ...
    if (!this.options.dev) {
      ...
      this.useMiddleware({
        ...
        handler: serveStatic__default["default"](distDir, this.options.render.dist)
      });
    }
    if (this.options.dev) {
  ...
  replaceMiddleware(query, middleware) {
    ...
    const { route, handle } = this.resolveMiddleware(middleware, serverStackItem.handle._middleware ? serverStackItem.handle._middleware._originalRoute : serverStackItem.route);
